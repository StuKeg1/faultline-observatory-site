...
import "./pages/EvidenceTrajectoriesOperationRegister.css";

function lazyRoute(importer) {
 return lazy(() => importer().then((module) => {
  if (typeof window !== "undefined") window.sessionStorage.removeItem("faultline:chunk-reload");
  return module;
 }).catch((error) => {
...
const DocumentaryImageryExperiment = lazyRoute(() => import("./pages/DocumentaryImageryExperiment.jsx"));

function NotFound() {
 return <><div style={{ padding: "4rem 2rem", fontFamily: "var(--serif)", color: "var(--ink-mid)", textAlign: "center" }}><div style={{ fontFamily: "var(--mono)", fontSize: "10px", letterSpacing: "0.1em", textTransform: "uppercase", marginBottom: "1rem" }}>404 — Not found</div><p>This record does not exist. <Link to="/" style={{ color: "var(--accent)" }}>Return to the Observatory.</Link></p></div><SiteFooter /></>;
}
...
function RecordRedirect() {
 ...
 return <ErrorBoundary context="record"><FrontierRecord /></ErrorBoundary>;
}

export function AppShell() {
 return <><ScrollToTop /><SiteNav /><Suspense fallback={null}><Routes>
  <Route path="/" element={<Home />} />
  <Route path="/public-record" element={<ErrorBoundary context="public record"><PublicRecord /></ErrorBoundary>} />
  <Route path="/reading-room" element={<ErrorBoundary context="reading room"><ReadingRoom /></ErrorBoundary>} />
  <Route path="/the-record" element={<ErrorBoundary context="archive"><TheRecord /></ErrorBoundary>} />
  <Route path="/evidence-trajectories" element={<ErrorBoundary context="evidence trajectories"><EvidenceTrajectories /></ErrorBoundary>} />
  <Route path="/evidence-trajectories/" element={<ErrorBoundary context="evidence trajectories"><EvidenceTrajectories /></ErrorBoundary>} />
  <Route path="/the-record/:recordId" element={<RecordRedirect />} />
  <Route path="/the-record/:recordId/" element={<RecordRedirect />} />
  <Route path="/events" element={<ErrorBoundary context="events"><EventsIndex /></ErrorBoundary>} />
  <Route path="/events/:eventId" element={<ErrorBoundary context="event"><EventDetail /></ErrorBoundary>} />
  ...
  <Route path="/political-rumour-accuracy-pilot/" element={<PoliticalRumourAccuracyPilot />} />
  <Route path="/experiments/vd-001" element={<ErrorBoundary context="VD-001 experiment"><VisualDiscoverabilityExperiment /></ErrorBoundary>} />
  <Route path="/experiments/vd-001/" element={<ErrorBoundary context="VD-001 experiment"><VisualDiscoverabilityExperiment /></ErrorBoundary>} />
  <Route path="/experiments/vd-002" element={<ErrorBoundary context="VD-002 experiment"><DocumentaryImageryExperiment /></ErrorBoundary>} />
  <Route path="/experiments/vd-002/" element={<ErrorBoundary context="VD-002 experiment"><DocumentaryImageryExperiment /></ErrorBoundary>} />
